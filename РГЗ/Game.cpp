...
Game::Game()
{
	...
	setColors();
	arrayOfColors;
	for (int i = 0; i < 7; i++) // создаем массив соответстви€ каждому возможному значению в квадрате определенного цвета
	{
		col[i].z = pow(2, i + 1);
...
void Game::GameRes(sf::RenderWindow &window, int a) {
	...
	if (a == 1) { //проверем были измено положение квадратов или была просто проведена кака€-либо работа с окном. ¬ этом случае звук не включаетс€, а интерфейс пол€ отрисовываетс€
